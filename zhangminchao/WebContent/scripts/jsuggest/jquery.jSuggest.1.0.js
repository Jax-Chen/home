 ...
 * edit by xujunjiang 2012/06/27 修改对结果的处理方式
	...
	$.fn.jSuggest = function(options) {
		...
		$(this).bind("keyup click", function(e){
			...
			if (this.value.length >= opts.minchar && $.trim(this.value)!="Search Terms") {
				var offSet = $(this).offset();//获取元素在当前视口的相对偏移
				$(jC).css({
					position: "absolute",
					top: offSet.top + $(this).outerHeight() + 2 + "px",
					left: offSet.left-2,
					width: $(this).outerWidth(true) + 2+2+2+"px",
				...
				// new query detected
					...
					iniVal = textBox.value;
					if (opts.showLoading && $(".jSuggestLoading").length==0)
						$('<div class="jSuggestLoading"><img src="'+opts.imgRoot+opts.loadingImg+'" align="bottom" /> '+ opts.loadingText+'</div>').prependTo("#jSuggestContainer");
...
//						opts.data = opts.data + "=" + $(this).val();
					...
					if(isEmpty(opts.data) || opts.data == '*:*'){
						return;
					}
					...
					setTimeout(function () {
						$.ajax({
							...
							success: function(json){
								...
								var dom = "<ul>";
								$.each(json.suggests, function(i, term) {
									dom +="<li>" + term + "</li>";
								});
								dom += "</ul>";
								$(jC).append(dom);
