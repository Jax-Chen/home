$(function(){
	...
	$("body").on("click",".delete",function(){
		...
		layer.confirm('确定删除？', {icon: 3, title:'提示'}, function(cindex){
            $.ajax({
                url: appServer+'/bops/user/delete.do',
                type: "POST",
                data: {"userId": userId},
//                beforeSend:function(){
...
//                    layer.close(loadIndex);
	...
	$("body").on("click",".user-add",function(){
		 ...
		 $.ajax({
             url: appServer+'/bops/home/listAllHome.do',
             type: "POST",
             success:function(ret){
             	var res = JSON.parse(ret);
	             if(res.meta.success){
	                        ...
	                        '<div class="col-sm-4">'+
	                        	'<select type="text" class="form-control" id="i-homeId" style="font-size:13px;">';
			                        	for(var i=0; i<home.length; i++){
					                        var nobj = home[i];
					                        ctt += '<option value="' + nobj.id + '">' + nobj.name + '</option>';
					                	}
	                  ...
	                  ctt += 	'</select>'+
	                        '</div>'+
	                	        ...
	                	        '<div class="col-sm-4">'+
	                		        ...
	                		        "<input type='text' id='i-name' value='' class='form-control' style='float:left;width:100%;'>"+
	                	    ...
	                	    '</div>';
	                		  ...
	                		  ctt += '<div class="form-group" style="margin-top:25px;">'+
	                            ...
	                            '<div class="col-sm-4">'+
	                                '<input type="text" class="form-control" id="i-uname" value="">'+
	                            '</div>'+
	                            
	                            
	                            '<label style="margin-top:0" class="col-sm-2 control-label"><span style="color:red;">*</span>注册手机</label>'+
	                            '<div class="col-sm-4">'+
	                                '<input type="text" class="form-control" id="i-mobile" value="">'+
	                            '</div>'+
	                        ...
	                        '</div>';
	                          ...
	                          '<div class="col-sm-4">'+
	                              '<input type="password" class="form-control" id="i-password" value="">'+
	                          '</div>'+
	                          
	                          
	                          '<label style="margin-top:0" class="col-sm-2 control-label"><span style="color:red;">*</span>密码确认</label>'+
	                          '<div class="col-sm-4">'+
	                              '<input type="password" class="form-control" id="i-password2" value="">'+
	                          '</div>'+
	                      ...
	                      '</div>';
	                          ...
	                          '<div class="col-sm-4">'+
	                              '<input type="text" class="form-control" id="i-email" value="">'+
	                          '</div>'+
	                          
	                          '<label style="margin-top:0" class="col-sm-2 control-label"><span style="color:red;"></span><span style="color:red;">*</span>性&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;别</label>'+
	                          '<div class="col-sm-4" style="">'+
	                          	'<label> <input type="radio" class="i-checks" checked value="1" name="i-sex"> <i></i> 先生</label>'+
	                          	"&nbsp;&nbsp;&nbsp;&nbsp;"+
	                          	'<label> <input type="radio" class="i-checks"  value="0" name="i-sex"> <i></i> 女士</label>'+
	                          '</div>'+
	                      '</div>';
	                		  ...
	                		  ctt += '<div class="form-group" style="margin-top:25px;">'+
	                            ...
	                            '<div class="col-sm-4">'+
	                                '<input type="text" class="form-control" id="i-birthday" value="" onclick="laydate({istime: true, format: \'YYYY-MM-DD\'})">'+
	                            '</div>'+
	                            
	                            '<label style="margin-top:0" class="col-sm-2 control-label">座&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;机</label>'+
	                            '<div class="col-sm-4">'+
	                                '<input type="text" class="form-control" id="i-tel" value="">'+
	                            '</div>'+
	                        '</div>';
	                		  ...
	                		  ctt += '<div class="form-group" style="margin-top:25px;">'+
	                			          ...
	                			          '<div class="col-sm-10">'+
	                			              '<input type="text" class="form-control" id="i-remark" value="">'+
	                			          '</div>'+
	                			      '</div>';
	                	...
	                	layer.open({
	                	           ...
	                	           yes:function(index){
	                                   ...
	                                   layer.confirm('确定添加用户？', {icon: 3, title:'提示'}, function(cindex){
	                        			   $.ajax({
	                                           url:appServer+'/bops/user/add.do',
	                                           type:"POST",
	                                           data:{"homeId":homeId,"name":name,"uname":uname,"mobile":mobile,"password":password,"email":email,"sex":sex,"birthday":birthday,"tel":tel,"address":address,"remark":remark},
	                                           beforeSend:function(){
	                                               loadIndex = layer.load(1);
	                                           },
	                                           success:function(ret){
	...
	$("body").on("click",".edit",function(){
		...
		$.ajax({
	        url: appServer+'/bops/user/detail.do',
	        type: "POST",
	        data: {"userId": userId},
	        success:function(ret){
	        	var res = JSON.parse(ret);
	         ...
	         ctt +=     '</select>'+
	        	...
	        	layer.open({
	        	           ...
	        	           yes:function(index){
	                           ...
	                           layer.confirm('确定修改用户？', {icon: 3, title:'提示'}, function(cindex){
	                			   $.ajax({
	                                   url:appServer+'/bops/user/edit.do',
	                                   type:"POST",
