...
public class StringUtil {
    ...
    public static String getReplaceText(String text) {
        ...
        if (index > 0) {
        	...
        	buf.append(arr[0].charAt(0));
        	for (int i=1;i<arr[0].length();i++) {
        		buf.append("*");
        	}
        	buf.append("@").append(arr[1]);
        	return buf.toString();
        } else {
        	buf.append(text.substring(0, 3)).append("*****").append(text.substring(text.length()-3));
        	return buf.toString();
        }
    }
    ...
    public static String maskIdNumber(String idNumber) {
        ...
        StringBuffer buf = new StringBuffer();
        buf.append(idNumber.substring(0, 3)).append("*************").append(idNumber.substring(idNumber.length() - 2));
    	return buf.toString();
    }
    ...
    public static String getDownLoadResumeEncodeName(HttpServletRequest request, String pname) {
    	// 设置response的Header
		String userAgent = request.getHeader("User-Agent").toUpperCase();
		String filename = null;
		try {
			if (userAgent.indexOf("MSIE") > 0 || userAgent.indexOf("MOZILLA/5.0") >= 0) {  
			    filename = URLEncoder.encode(pname, "UTF-8");  
			}  else {
				filename = new String(pname.getBytes("UTF-8"), "ISO8859-1");  
			}
		} catch(Exception e) {
			filename = pname;
		}
	...
	public static String replaceTarget(String content, String regexp, String replacement) {
	...
	public static String replaceFirstTarget(String content, String regexp, String replacement) {
	...
	public static String strListToString(List<String> list, String split) {
		if (list == null) {
			return null;
		}
		StringBuilder result = new StringBuilder();
	...
	public static String replaceAll(String content, String expr, String substitute) {
		return	Pattern.compile(expr, Pattern.MULTILINE |Pattern.CASE_INSENSITIVE| Pattern.DOTALL).matcher(content).replaceAll(substitute);
	}
	...
	public static String formatRewardDetail(String content) {
...
//		/*String text = "就读于杭州电大";
//		System.out.println(text.split("就读于").length);
//		System.out.println(text.split("就读于")[0]);
//		System.out.println(text.split("就读于")[1]);*/
//		//String content = "This is 1. 2002 2、bb \r\n3. xx中国 <tr> applestart another appleend 44 pear  applestart 55 appleend ...applestart 66 appleend  </tr> ~~~~ applestart ppp appleend test.";
//		//System.out.println(replaceTarget(content, "<tr>.*?(applestart)(.*)(appleend).*?</tr>", "start$2end"));
//		//System.out.println(System.currentTimeMillis());
//		/*StringBuffer sb = new StringBuffer();
//		Pattern p = Pattern.compile("(<tr.*?>)(.*?applestart.*?)(</tr>)",
//				Pattern.MULTILINE | Pattern.DOTALL);
//		String r = content;
//		Matcher m = p.matcher(content);
//		while (m.find()) {
//			m.appendReplacement(sb, "$1"+m.group(2).replaceAll("applestart", "start").replaceAll("appleend", "end")+"$3");
//		//	String findContent = m .group(1);
//		//	String replaceContent = findContent.replaceAll("applestart", "start").replaceAll("appleend", "end");
//		//	r = StringUtil.replaceAll(r, findContent, replaceContent);
//		}
//		m.appendTail(sb);
//		System.out.println(sb.toString());*/
//		
//		//String text = "1452926050816_洪军   网贷.doc";
//		String text = "upload/resume_upload/20160116/1452926050816_洪军   网贷.doc";
//		System.out.println(text.replaceAll("[^\\da-zA-Z_\\-\\./\u4e00-\u9fa5]+", "_").replaceAll(".net", "_net"));
//		System.out.println(maskIdNumber("330321478507150069"));
	 ...
	 * @return
	 */
	public static boolean isContain(String[] extarray, String endfix) {
	 ...
	 * @param spl
	 */
	public static List<String> string2StringList(String str, String spl) {
	 ...
	 * @param spl
	 */
	public static List<Integer> string2IntegerList(String str, String spl) {
		String[] s = str.split(spl);
		if(s != null && s.length != 0){
